...
namespace fop
{
  ...
  void FrenetOptimalPlannerNode::processreferencepath()
  {
    ...
    data_saver_.AddData("runtime_control_loop", elapsed_time.count() / 1000.); // Save the runtime in seconds
  ...
  bool FrenetOptimalPlannerNode::calculateControlOutput(const int next_wp_id, const fop::VehicleState &frontaxle_state)
  {
    ...
    else
    {
      ...
      /*
      for (size_t i = 0; i < curr_trajectory_.v.size(); i++)
      {
        std::cout << "the velocity of the current trajectory is: " << curr_trajectory_.v[i] << std::endl;
      }
      */
